...
The canonical centroid algorithm. Given n points `{x_i}` from i=1 to n in R^d and k clusters, K-Means minimises the within-cluster sum of squares (WCSS):

```math
\underset{C}{\arg\min} \sum_{j=1}^{k} \sum_{x_i \in C_j} \|x_i - \mu_j\|_2^2
```

where `mu_j = (1/|C_j|) * sum_{x_i in C_j} x_i` is the centroid of cluster j. The Lloyd–Forgy algorithm alternates the **E-step** (nearest-centroid assignment):
...
Initialisation uses **K-Means++**: the first centroid is sampled uniformly; subsequent centroids c_l are sampled with probability `P(x_i)` proportional to `min_{j < l} ||x_i - c_j||^2`. This D² weighting yields the approximation guarantee:

```math
\mathbb{E}[\mathrm{WCSS}_{k\text{-means}++}] \leq 8(\ln k + 2)\cdot \mathrm{WCSS}_{\mathrm{OPT}}
```

- **Complexity:** Time O(nkd · T) per iteration T; Space O(kd)
...
Stochastic approximation that processes random subsets (mini-batches) of size b << n per iteration. The per-centre learning rate is `eta_{c_j} = 1/(1 + n_{c_j})` where `n_{c_j}` counts prior assignments, and the centre update is:

```math
\mu_j^{(t+1)} = \left(1 - \eta_{c_j}\right)\mu_j^{(t)} + \eta_{c_j}\cdot \overline{x}_{\mathcal{B},j}
```

This harmonic learning rate schedule satisfies the Robbins-Monro conditions `sum_t eta_t = infinity`, `sum_t eta_t^2 < infinity`, guaranteeing almost-sure convergence. The algorithm achieves near-equivalent quality to full K-Means at O(b) per step.

- **Complexity:** Time O(bkd) per step; Space O(kd)
...
- **Complexity:** Time O(k(n-k)^2) per iteration; Space O(n²) for distance matrix
...
- **Complexity:** Time O(nk · T · log k); Space O(n)
...
DBSCAN constructs clusters from density-connected regions defined by radius epsilon and minimum point count MinPts. A point p is a **core point** if `|N_epsilon(p)| >= MinPts`. Two core points are **density-connected** if there exists a chain `p = p_0, p_1, ..., p_m = q` with `p_{i+1}` in `N_epsilon(p_i)`. A cluster is the maximal density-connected set; border points are reachable from a core but not core themselves; noise points receive label -1.

```math
C_j = \bigl\{x \mid \exists\;\text{density-reachability path from some}\; c \in \mathrm{CorePoints}\;\text{to}\; x\bigr\}
```

The system estimates epsilon automatically via the **k-NN Distance Profile**: sorting k-th nearest-neighbour distances and locating the elbow via maximum curvature (kneedle algorithm).

- **Complexity:** Time O(n log n) with spatial indexing; Space O(n)
- **Parameters:** `eps` (epsilon), `min_samples` (MinPts), `metric`, `algorithm` {auto, ball_tree, kd_tree}
...
where `core_k(a) = d(a, kNN_k(a))` is the core distance. The minimum spanning tree of the mutual-reachability graph is extracted; the condensed hierarchy selects persistent clusters by maximising **cluster stability**:
...
The Excess of Mass (EOM) criterion selects the subset of clusters maximising total stability. HDBSCAN requires no epsilon parameter and produces soft cluster probabilities.
...
The reachability plot reveals cluster structure as valleys; cluster extraction applies xi-steep descent detection or a global epsilon threshold. The xi-method requires no global density threshold, revealing multi-scale structure.

- **Complexity:** Time O(n²) worst case, O(n log n) with indexing; Space O(n)
...
Non-parametric mode-seeking algorithm. Each point iteratively migrates toward the mean of points within bandwidth h:
...
with Gaussian kernel `K(u) = exp(-||u||^2 / 2)`. The mean shift vector `m(x) - x` is proportional to the gradient of the kernel density estimate `nabla f_hat_h(x)`, so updates climb the density surface. With bandwidth h estimated via Silverman's rule, the algorithm requires no k.

- **Complexity:** Time O(Tn²); Space O(n)
...
Bottom-up linkage clustering merges the closest pair at each step. For clusters A, B:

| Linkage | Distance D(A,B) | Behaviour |
|---------|-----------------|-----------|
| **Ward** | `n_A * n_B / (n_A + n_B) * ||mu_A - mu_B||^2` | Minimises intra-cluster variance increase |
| **Complete** | `max_{a in A, b in B} d(a,b)` | Penalises outliers; compact clusters |
| **Average (UPGMA)** | `(1/|A||B|) * sum_{a in A} sum_{b in B} d(a,b)` | Compromise; less outlier-sensitive |
| **Single** | `min_{a in A, b in B} d(a,b)` | Chaining effect; detects elongated shapes |
...
All linkages are unified by the **Lance–Williams recurrence** for updating distances to the merged cluster `A ∪ B` from existing cluster C:

```math
D(A \cup B,\, C) = \alpha_A\, D(A,C) + \alpha_B\, D(B,C) + \beta\, D(A,B) + \gamma\, |D(A,C) - D(B,C)|
```

with family-specific coefficients. This avoids recomputing all pairwise distances, enabling O(n² log n) priority-queue implementations.

- **Complexity:** Naïve O(n³); with Lance–Williams and priority queue O(n² log n)
...
BIRCH uses a **Clustering Feature (CF)** tree to summarise data incrementally. Each CF node stores (n, LS, SS): count, linear sum, and squared sum. Merging two CFs is O(1):
...
- **Complexity:** Time O(n); Space O(tree size); single-pass
...
Transposed agglomerative approach: clusters *features* (columns) rather than samples using Ward linkage on the feature correlation structure. Grouped features are replaced by their mean, enabling clustering in semantically coherent feature spaces. The feature affinity matrix `(Sigma_F)_{ij} = rho(f_i, f_j)` defines the feature space.
...
1. Affinity matrix: `W_{ij} = exp(-||x_i - x_j||^2 / 2*sigma^2)`
...
3. Solve `L_sym * u = lambda * u`; collect k smallest eigenvectors into `U` in R^{n x k}
4. Row-normalise U and apply K-Means

The **spectral gap** `delta_k = lambda_{k+1} - lambda_k` reveals natural cluster count: under ideal separation, `lambda_1 = ... = lambda_k = 0` and `lambda_{k+1} > 0`. The multiplicity of eigenvalue 0 of the unnormalised Laplacian equals the number of connected components. The `discretize` assignment variant rounds the embedding matrix via iterative rotation:

```math
\min_{R \in \mathcal{O}(k)} \|U - VR\|_F, \quad V_{ij} = u_{ij}/\|u_i\|_2
```

- **Complexity:** Time O(n³) for dense W; Space O(n²)
...
EM monotonically increases the log-likelihood `L = sum_i log p(x_i)` at every iteration (by Jensen's inequality applied to the Q-function lower bound). BIC for model selection:
...
- **Complexity:** Time O(nkd² · T); Space O(kd²)
...
The Dirichlet concentration alpha controls sparsity: as `alpha -> 0`, most components receive `pi_j ≈ 0` (automatic model order selection). The `dirichlet_process` prior encourages parsimonious solutions.
...
Message-passing algorithm. Similarity `s(i,k) = -||x_i - x_k||^2` (negative squared Euclidean by default).

**Responsibility** `r(i,k)` — evidence that k should serve as exemplar for i:

```math
r(i,k) \leftarrow s(i,k) - \max_{k' \neq k}\bigl\{a(i,k') + s(i,k')\bigr\}
```

**Availability** `a(i,k)` — evidence that i should choose k:
...
Exemplars identified where `a(k,k) + r(k,k) > 0`. Damping factor `lambda` in [0.5, 1) stabilises oscillations: `r <- (1-lambda)*r_new + lambda*r_old`.
...
For each point i in cluster C_j: `a(i)` = mean intra-cluster distance; `b(i) = min_{l != j} d_bar(i, C_l)` = mean distance to nearest other cluster.

```math
s(i) = \frac{b(i) - a(i)}{\max\!\left(a(i),\, b(i)\right)}, \qquad S = \frac{1}{n}\sum_{i=1}^n s(i) \in [-1, 1]
```

`s(i) = 1` indicates perfect cohesion (`a(i) -> 0`); `s(i) = -1` indicates misassignment (`b(i) -> 0`). Interpretation thresholds: [0.70, 1] strong; [0.50, 0.70) reasonable; [0.25, 0.50) weak; < 0.25 no structure.
...
where `sigma_j = (1/|C_j|) * sum_{x in C_j} d(x, mu_j)`. DBI rewards compact clusters (small `sigma_j`) that are well-separated (large `d(mu_j, mu_l)`). Lower is better; optimal clusters score 0.
...
where the between-cluster scatter matrix is `B_k = sum_j n_j * (mu_j - mu_bar)(mu_j - mu_bar)^T` and the within-cluster scatter matrix is `W_k = sum_j sum_{x in C_j} (x - mu_j)(x - mu_j)^T`. The total scatter `T = B_k + W_k` is constant; maximising CH is equivalent to maximising the ratio of explained-to-unexplained variance. Higher is better.
...
where `delta(C_i, C_j) = min_{x in C_i, y in C_j} d(x,y)` is the inter-cluster minimum distance and `Delta(C_k) = max_{x,y in C_k} d(x,y)` is the cluster diameter. Higher values indicate compact, well-separated clusters. DI is sensitive to outliers (a single outlier inflates Delta).
...
Compactness-to-separation ratio; lower is better. The numerator is WCSS; the denominator is n times the minimum squared centroid separation.
...
where `Scat(k) = (1/k) * sum_j ||sigma_j|| / ||sigma||` measures average cluster scatter relative to dataset scatter, and

```math
\mathrm{Dens\_bw}(k) = \frac{1}{k(k-1)}\sum_{i \neq j} \frac{\rho(u_{ij})}{\max\!\left(\rho(\mu_i),\,\rho(\mu_j)\right)}
```

measures density at mid-points `u_{ij} = (mu_i + mu_j)/2` relative to centroid densities. Both terms should be minimised.
...
where `W_k = sum_j (1 / 2*n_j) * D_j` is pooled within-cluster dispersion. The expectation is over B uniformly drawn reference datasets. Optimal k satisfies:
...
where `u_i` = nearest-neighbour distance from a uniformly random point to data, and `w_i` = nearest-neighbour distance from a data point to another data point. Under a uniform distribution, `H ~ Beta(m,m)` with mean 0.5. `H -> 1` indicates strong clustering tendency. The null hypothesis H_0 (uniform distribution) is rejected when `H > 0.75`.
...
A balanced clustering (`n_j = n/k` for all j) achieves maximum entropy `log_2 k`; a degenerate single-cluster result gives `H = 0`. Used internally to detect imbalanced assignments.
...
where `v_tilde_m` normalises metric m to [0,1] (respecting direction) and `w_m` are predefined weights (Silhouette: 3, DBI: 2, CH: 1.5, Dunn: 1). A pairwise win matrix counts how many metrics each algorithm "wins" against every other.
...
2. **Data Profiling** — per-column statistics (dtype, unique count, missing%, mean, std, skewness `g_1 = mu_3/mu_2^{3/2}`, excess kurtosis `g_2 = mu_4/mu_2^2 - 3`, outlier%), high-correlation pair detection (r > 0.9)
3. **Missing Value Imputation** — `mean`, `median`, `most_frequent`, `knn` (k-NN), `iterative` (MICE), `constant`, `drop_rows`, `drop_cols`
4. **Outlier Detection & Handling**
   - Z-score: flag `|z_i| > 3` where `z_i = (x_i - mu)/sigma`
...
7. **Feature Selection**
   - Variance threshold: `Var(f) < tau` → drop
   - Correlation filter: `|rho(f_i, f_j)| > theta` → drop one
   - PCA: retain v% explained variance
   - ANOVA F-test feature ranking
8. **Column Dropping** — user-specified columns (e.g. ground-truth labels)

### k-NN Distance Profile for DBSCAN epsilon Estimation
...
This discrete approximation to curvature `kappa ≈ |y''|` identifies the point of maximum rate-of-change in the sorted distance curve — the optimal density threshold for DBSCAN.
...
The `StabilityPipeline` resamples the dataset B times (subsample ratio rho), re-runs the algorithm, and measures consistency via **Adjusted Rand Index (ARI)**:

```math
\mathrm{ARI} = \frac{\mathrm{RI} - \mathbb{E}[\mathrm{RI}]}{\max(\mathrm{RI}) - \mathbb{E}[\mathrm{RI}]}
```

where the Rand Index `RI = (a + d) / C(n,2)` counts concordant pairs (a: same cluster in both; d: different clusters in both). ARI ranges in [-1, 1] with expected value 0 for random labelling and 1 for perfect agreement. **Jaccard Index**:
...
The `ConsensusPipeline` builds the **co-association matrix** over M runs:

```math
A_{ij} = \frac{\text{number of runs where } x_i,\, x_j \text{ co-cluster}}{\text{number of runs where both appear}}
```

`A` in [0,1]^{n×n} symmetric; final clusters are obtained by agglomerative clustering (average linkage) on `1 - A`.
...
where `F(theta)` is the empirical CDF of A entries. Optimal k minimises PAC — values near 0 indicate decisive (non-ambiguous) co-association.

**Cophenetic Correlation**: Pearson correlation between co-association distances `(1 - A_{ij})` and dendrogram cophenetic distances; high values (>0.9) indicate the hierarchy faithfully represents the consensus matrix.

### Perturbation Analysis

Gaussian noise `x_tilde_i = x_i + epsilon`, `epsilon ~ N(0, sigma^2 * I)` at levels `sigma_noise` in {0.05, 0.10, 0.20, 0.30}; ARI is measured against the clean-data result. Robustness score = mean ARI across noise levels.
...
Incremental data checkpoints (0.1, 0.2, …, 1.0) are clustered; ARI between consecutive checkpoints measures stability as data accumulates — a key property for streaming deployments.
...
| `NNDistancePlotter.plot` | k-NN distance curve with epsilon suggestion |
| `HopkinsPlotter.plot` | Hopkins H gauge |
| `MetricsTablePlotter.plot` | Colour-coded normalised comparison table |

**Dimensionality Reduction Methods:**

**PCA** — Linear projection: `Z = X * W_k` where `W_k` in R^{d×k} contains the top-k eigenvectors of `X^T X`. Explained variance ratio: `EVR_l = lambda_l / sum_i lambda_i`.

**t-SNE** — KL divergence minimisation between pairwise similarity distributions. High-dimensional similarities (perplexity controls `sigma_i`):
...
Minimised cost: `L_KL = KL(P || Q) = sum_{i != j} p_{ij} * log(p_{ij}/q_{ij})`.
