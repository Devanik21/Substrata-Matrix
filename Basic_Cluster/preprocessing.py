...
Data loading, profiling, imputation, outlier detection, scaling, 
encoding, feature selection, and pipeline assembly.
"""
...
def describe_pipeline(config: PreprocessingConfig) -> str:
    """Return a human-readable description of the preprocessing pipeline."""
    steps = [
        f"1. Imputation: {config.impute_strategy.value}",
        f"2. Scaling: {config.scaler_type.value}",
        f"3. Outlier handling: {config.outlier_method.value} (contamination={config.outlier_contamination})",
        f"4. Feature selection: {config.feature_selection.value} (max={config.max_features})",
        f"5. Variance threshold: {config.variance_threshold}",
    ]
    return "\n".join(steps)
